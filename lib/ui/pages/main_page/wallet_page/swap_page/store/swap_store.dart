...
abstract class SwapStoreBase extends IStore<SwapStoreState> with Store {
    ...
    try {
      ...
      final _hashTx = await _client.sendTransaction(
        ...
        Transaction.callContract(
          ...
          gasPrice: _gas,
          parameters: _setParameters(nonce: _nonce, degree: 18),
        ),
  ...
  approve() async {
    try {
      ...
      final _txHashApprove = await contract.approve(
        _spender,
        (Decimal.parse(amount.toString()) * Decimal.fromInt(10).pow(18).toDecimal()).toBigInt(),
        credentials: _cred,
  ...
  Future<bool> needApprove() async {
    ...
    final _amount = BigInt.from(amount * pow(10, 18));
